...
router.post('/:id/close', requirePermission('fleet', 'update'), validate(closeMaintenanceSchema), async (req, res, next) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const log = await tx.maintenanceLog.findFirst({ where: { id: parseInt(req.params.id as string, 10), ...branchFilter, deletedAt: null } });
...
router.delete('/:id', requirePermission('fleet', 'delete'), async (req, res, next) => {
  try {
    const branchFilter = req.user!.role === 'SUPER_ADMIN' ? {} : { branchId: req.user!.branchId! };
    const log = await prisma.maintenanceLog.findFirst({ where: { id: parseInt(req.params.id as string, 10), ...branchFilter, deletedAt: null }});
